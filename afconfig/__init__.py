...
def get_config_value(config, *keys, **kwargs):
    ...
    """
    if not keys:
        raise ConfigurationError(NO_KEYS_ERR_MSG)
...
def set_config_value(config, value, *keys):
    if not isinstance(config, dict):
        raise ConfigurationError(INVALID_CONFIG_ERR_MSG)
    if not keys:
        raise ConfigurationError(MISSING_KEYS_ERR_MSG)
...
def merge_configs(config, to_be_merged_config):
    ...
    for k, v in to_be_merged_config.items():
        if k not in config or (
            ...
            raise ConfigurationError(CONFIG_CONFLICT)
